<template>
	<view>
		<view class="order-item" v-for="(item, index) in listData" :key="index" @click="openPage('/pages/teacher/homepage')">
			...
			<view class="button-group" v-if="item.status == 1 || item.status == 4">
				...
				<u-button type="warning" v-if="item.status == 4 && !item.is_evaluated" text="我要评价"></u-button>
				<u-button type="warning" v-if="item.status == 4 && item.start_time==0" text="结束订单"></u-button>
			</view>
			<view class="order-residue-time" v-if="item.status == 3">
